...
def main():
    ...
    if (not cfg.is_test and not cfg.is_train and not cfg.is_use):
        print("pass either --test or --train as args, and optionally --cfg path/to/config.ini if config/wind_field_GAN_2D_config.ini isn't what you're planning on using.")
        return 
...
def argv_to_cfg() -> Config:
    ...
    parser.add_argument("--cfg", type=str, default="config/wind_field_GAN_2D_config.ini", help="path to config ini file (defaults to /config/wind_field_GAN_2D_config.ini)")
    ...
    if is_use:
        cfg_path = os.path.dirname(os.path.realpath(__file__)) + "/config/config_use.ini"
        print(cfg_path)
