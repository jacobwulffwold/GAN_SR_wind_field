...
def test(cfg: config.Config):
    ...
    if cfg.model.lower() != "wind_field_GAN_2D":
        raise ValueError(f"Only wind_field_GAN_2D is supported - not model {cfg.model}")
    gan = wind_field_GAN_2D.wind_field_GAN_2D(cfg)
        ...
        with open(metrics_file, "w") as f:
            ...
            for j, data in enumerate(dataloader_test):
                ...
                for i in range(len(lrs)):
                    ...
                    imgs = make_and_write_images(lr_i, hr_i, sr_i, cfg.env.this_runs_folder, j, cfg.scale)

                    write_metrics(imgs, j, f)


def make_and_write_images(lr: torch.Tensor, hr: torch.Tensor, sr: torch.Tensor, folder_path: str, img_name: int, scale: int) -> dict:
    ...
    imgs_bicubic = nn.functional.interpolate(lr, scale_factor=4, mode='bicubic', align_corners=True).squeeze().detach().cpu().numpy()
...
def img_psnr(sr, hr) -> float:
    ...
    MSE = MSE.item()
    R_squared = np.max(hr)**2#255.0*255.0 # R is max fluctuation, and data is cv2 img: int [0, 255] -> R² = 255²
