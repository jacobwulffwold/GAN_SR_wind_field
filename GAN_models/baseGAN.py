...
class BaseGAN(lc.GlobalLoggingClass):
    ...
    def load_model(
        ...
        if (
            not generator_load_path is None
        ...
        if (
            ...
            and not discriminator_load_path.lower() == "null"
        ...
        if (
            ...
            assert len(loaded_optimizers) == len(
                self.optimizers
            ), f"Loaded {len(loaded_optimizers)} optimizers but expected { len(self.optimizers)}"
            assert len(loaded_schedulers) == len(
                self.schedulers
            ), f"Loaded {len(loaded_schedulers)} schedulers but expected {len(self.schedulers)}"
