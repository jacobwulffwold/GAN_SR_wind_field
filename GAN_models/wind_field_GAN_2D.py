...
from CNN_models.Discriminator import Discriminator_2D
from CNN_models.Generator import Generator_2D
...
class wind_field_GAN_2D(BaseGAN):
    ...
    def __init__(self, cfg: config.Config):
        ...
        if cfg.is_train:
            ...
            # elif cfg_t.feature_criterion == "l1":
            #     self.feature_criterion = nn.L1Loss()
            # elif cfg_t.feature_criterion == "l2":
            #     self.feature_criterion = nn.MSELoss()
            # else:
            #     raise NotImplementedError(
            #         f"Only l1 and l2 (MSE) loss have been implemented for feature loss, not {cfg_t.feature_criterion}"
            #     )
    ...
    def compute_losses_and_optimize(
        ...
        if training_epoch:
            ...
            grad_start = (
                self.D.features[0][0][0].weight.grad.detach().cpu()
            )
