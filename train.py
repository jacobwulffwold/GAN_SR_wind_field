...
def train(cfg: config.Config):
    ...
    dataset_train, dataset_test, dataset_validation = preprosess()
    ...
    if cfg.dataset_train:
        dataloader_train = torch.utils.data.DataLoader(dataset_train, batch_size=cfg.dataset_train.batch_size,
                                          shuffle=True, num_workers=cfg.dataset_train.n_workers, pin_memory=True)
    ...
    if cfg.dataset_val:
        dataloader_val = torch.utils.data.DataLoader(dataset_validation, batch_size=cfg.dataset_val.batch_size,
                                         shuffle=False, num_workers=cfg.dataset_val.n_workers, pin_memory=True)
    ...
    if cfg.model.lower() != "wind_field_gan_2d":
        status_logger.info(f"only wind_field_GAN_2D (wind_field_GAN_2D) is supported at this time - not {cfg.name}")
    ...
    bar = displaybar.DisplayBar(max_value=len(dataloader_train), start_epoch=start_epoch, start_it=it, niter=cfg_t.niter)
            ...
            if it % cfg_t.val_period == 0:
                ...
                for _, val_data in enumerate(dataloader_val):
                    ...
                    if (i_val % len(dataloader_val) == 0):
                        with torch.no_grad():
                            batch_quiver = torch.LongTensor(np.asarray([np.random.randint(val_LR.shape[0], size=1)]))[0].to(cfg.device) #
                            ...
                            u_LR = LR_ori[0,:,:]#*(np.max(u_nomask)-np.min(u_nomask))+np.min(u_nomask)
                            v_LR = LR_ori[1,:,:]#*(np.max(v_nomask)-np.min(v_nomask))+np.min(v_nomask)
                            w_LR = LR_ori[2,:,:]#*(np.max(w_nomask)-np.min(w_nomask))+np.min(w_nomask)
                            ...
                            imgs_bicubic = nn.functional.interpolate(LR_i, scale_factor=4, mode='bicubic', align_corners=True).squeeze().detach().cpu().numpy()  
                            u_bicubic = imgs_bicubic[0,:,:]#*(np.max(u_nomask)-np.min(u_nomask))+np.min(u_nomask)
                            v_bicubic = imgs_bicubic[1,:,:]#*(np.max(v_nomask)-np.min(v_nomask))+np.min(v_nomask)
                            w_bicubic = imgs_bicubic[2,:,:]#*(np.max(w_nomask)-np.min(w_nomask))+np.min(w_nomask)
                            ...
                            u_sr = gen_HR[0,:,:]#*(np.max(u_nomask)-np.min(u_nomask))+np.min(u_nomask)
                            v_sr = gen_HR[1,:,:]#*(np.max(v_nomask)-np.min(v_nomask))+np.min(v_nomask)
                            w_sr = gen_HR[2,:,:]#*(np.max(w_nomask)-np.min(w_nomask))+np.min(w_nomask)

                            # HR
                            HR = HR_i.squeeze().detach().cpu().numpy() #torch.index_select(val_HR, 0, batch_quiver, out=None)
                            u_HR = HR[0,:,:]#*(np.max(u_nomask)-np.min(u_nomask))+np.min(u_nomask)
                            v_HR = HR[1,:,:]#*(np.max(v_nomask)-np.min(v_nomask))+np.min(v_nomask)
                            w_HR = HR[2,:,:]#*(np.max(w_nomask)-np.min(w_nomask))+np.min(w_nomask)
                            ...
                            with open(cfg.env.this_runs_folder + "/images/val_imgs__it_" + str(it) + ".pkl", 'wb') as f:
                                pkl.dump(imgs, f)
