...
for epoch in range(num_epochs): #Loop over the dataset multiple times
    ...
    for i, data in enumerate(trainloader):
        ...
        output = netD(HR.float()) # We forward propagate this real image into the neural network of the discriminator to get the prediction (a value between 0 and 1).
        errD_HR = criterion(output, target) # We compute the loss between the predictions (output) and the target (equal to 1).
        
        # Training the discriminator with a fake image generated by the generator
        LR = Variable(LR)
        target = Variable(torch.zeros(HR.size()[0])) # We get the target.
        output = netD(LR.float().detach()) # We forward propagate the fake generated images into the neural network of the discriminator to get the prediction (a value between 0 and 1).
        errD_LR = criterion(output, target) # We compute the loss between the prediction (output) and the target (equal to 0).
        ...
        target = Variable(torch.ones(HR.size()[0])) # We get the target.
        output = netD(LR) # We forward propagate the fake generated images into the neural network of the discriminator to get the prediction (a value between 0 and 1).
