...
class DisplayBar:
    def __init__(
        ...
        self.progressbar_it_epoch = progressbar.FormatCustomText(
            "[It: %(it)6d / %(niter)6d] [Epoch: %(epoch)4d (%(current_value)5d/%(max_value)5d)]",
            dict(
                epoch=start_epoch,
                it=start_it,
                niter=niter,
        ...
        self.bar = progressbar.ProgressBar(
            max_value=max_value, widgets=self.progressbar_widgets
        )

    def update(self, current_value: int, current_epoch: int, current_it: int):
        self.progressbar_it_epoch.update_mapping(
            epoch=current_epoch, it=current_it, current_value=current_value
        )
